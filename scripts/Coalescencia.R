...
link <- "https://github.com/felipelirapaiva/dissertacao/blob/main/mydata/gabinetes_comideologia.csv?raw=true"
download.file(link, "gabinetes_comideologia.csv")
coalizoes <- read.csv("gabinetes_comideologia.csv", dec = ",", encoding = "UTF-8", check.names = FALSE)
...
# Download da base de deputados
link <- "https://github.com/felipelirapaiva/dissertacao/blob/main/mydata/valores_NEPP.csv?raw=true"
download.file(link, "valores_NEPP.csv")
valores_NEPP <- read.csv("valores_NEPP.csv", dec = ",", encoding = "UTF-8", check.names = FALSE)

# Dando um merge
  library(scales)

gabinetes_final <- left_join(coalizoes, valores_NEPP,
                                    by = c("ANO_REFERENCIA" = "ANO_ELEICAO",
                                           "SIGLA_UE" =  "UF"))

write_csv(gabinetes_final,"C:\\Users\\Felipe\\Documents\\Dissertacao R\\Diss\\gabinetes_final.csv")